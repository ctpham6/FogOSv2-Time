    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          if(utime != 0 && copyout(p->pagetable, utime, (char *)&pp->user_time,
                                  sizeof(pp->user_time)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
          }
          if(ktime != 0 && copyout(p->pagetable, ktime, (char *)&pp->kern_time,
                                  sizeof(pp->kern_time)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
          }
